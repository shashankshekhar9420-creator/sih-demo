...
import { ArrowLeft, ArrowRight, ArrowUpRight, Camera, Check, CheckCheck, CircleHelp, Image, Leaf, Mic, Package, Save, ShieldCheck, Sparkles, Store, Tag } from 'lucide-react';
import { categories, categorySchema, moneySchema, rupees, stockSchema } from '@sahaj/shared';
import type { Catalog } from '../lib/catalog';
import { catalogResponseSchema, dateLabel, jsonRequest, request, RequestError, resumeStep, safeProductUrl, type ReviewValues } from './client-api';
...
export function CatalogWizard({ id }: { id: string }) {
  ...
  return <div className="wizard">
    <div className="wizard-topline"><Link href="/catalogs" className="text-link"><ArrowLeft size={16} /> My catalogs</Link><div><span className="saved-indicator"><Check size={14} />{busy ? 'Saving...' : dirty ? 'Unsaved changes' : `Saved ${dateLabel(catalog.updatedAt)}`}</span><StatusBadge status={catalog.status} /></div></div>

...
export function VoiceRecorder({ catalog, busy, onTranscribe, onContinue, onDirty }: {
  ...
  const isPlaceholder = catalog.englishTranslations.some(t => t.toLowerCase().includes('demo placeholder'));
  const langNames: Record<string, string> = {
    hi: 'Hindi', en: 'English', ta: 'Tamil', te: 'Telugu', kn: 'Kannada',
    ml: 'Malayalam', bn: 'Bengali', gu: 'Gujarati', pa: 'Punjabi', mr: 'Marathi', ur: 'Urdu',
  };
  const sourceLangName = langNames[catalog.sourceLanguage ?? 'hi'] || catalog.sourceLanguage?.toUpperCase() || 'Hindi';

  return <div className="voice-step">
    {isPlaceholder && <DemoNotice>Voice transcription is in demo mode: each question currently holds placeholder text. Record your answers to generate real speech transcripts and English translations.</DemoNotice>}
    ...
    {catalog.englishTranslations.length === 3 && <section className="transcript-section"><div className="section-heading"><div><span className="eyebrow">ONE RESULT FOR EACH QUESTION</span><h2>Your saved voice results</h2></div><span className="badge">Source: {sourceLangName}</span></div>{questions.map((item, index) => <details className="transcript-card" key={item.title} open={!isPlaceholder}><summary><span>0{index + 1}</span> {item.title}<span className={`badge ${!isPlaceholder ? 'badge-published' : ''}`}>{!isPlaceholder ? 'Transcribed' : 'Demo result'}</span></summary><div className="bilingual-grid"><div><span className="field-caption">{catalog.sourceLanguage === 'hi' ? 'HINDI TRANSCRIPT' : `${(langNames[catalog.sourceLanguage ?? 'hi'] || catalog.sourceLanguage || 'ORIGINAL').toUpperCase()} TRANSCRIPT`}</span><p lang={catalog.sourceLanguage || 'hi'}>{catalog.regionalTranscripts[index]}</p></div><div><span className="field-caption">ENGLISH TRANSLATION</span><p>{catalog.englishTranslations[index]}</p></div></div></details>)}</section>}
